    ...
    if circles is not None: # HoughCircles가 원을 하나 이상 찾았을 때만 루프.
        for c in circles[0, :]:
            ...
            if (x2 - x1) > 30 and (y2 - y1) > 30: # 크기가 30×30보다 작은 후보는 버린다 (노이즈 후보 제거).
                ...
                edges = cv2.Canny(roi, 80, 160) # Canny 엣지 검출로, 이 영역에 선/엣지가 얼마나 많은지 확인. 휠/타이어 부분은 보통 스포크, 패턴 때문에 엣지가 풍부하다고 가정.
                scores.append(float(edges.mean()) / 255.0) #  엣지 영상의 평균 밝기를 계산한다. 이는 0~255 사이인데 나누기 255.0으로 0~1 사이로 정규화. 엣지 많을수록 평균이 높아지므로, 엣지가 많은 후보일수록 점수 높게 설정.
